document.addEventListener("DOMContentLoaded", () => {
  ...
  function draw() {
    ...
    for (let i = 0; i < nodes; i++) {
      const x = offsetX + Math.sin(i + time) * (width / 3) * Math.sin(time + i * 0.5);
    ...
    // Optional: Draw mirrored side for projects view
    if (view === "projects") {
      for (let i = 0; i < nodes; i++) {
        const x = width * 0.75 + Math.sin(i + time) * (width / 3) * Math.sin(time + i * 0.5);
        const y = height / 2 + Math.cos(i + time) * (height / 3) * Math.cos(time + i * 0.5);
        const r = 2 + Math.sin(time + i) * 1.5;
